...
gmres_solve(const SpeciesJacobian& A, const Species& b, double abs_tol = 1e-8, double rel_tol = 1e-4) 
{
    ...
    std::array<std::array<double, n_species /* columns */>, n_species + 1 /* rows */> H = {}; // Hessenberg matrix that's modified in place at each iteration to yield the upper triangular (rectangular) matrix
