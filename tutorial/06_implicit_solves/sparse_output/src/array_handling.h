...
SpeciesJacobian operator*(const double& s, const SpeciesJacobian& a) 
{
    return scale_gen(s, a);
}
...
#if defined(CHEMGEN_EIGEN)
void
add_species_to_jacobian_row_eigen_sparse(std::vector<Triplet_>& triplet, int row_index, const Species& b_s)
{
    for (int j = 0; j < n_species; ++j) triplet.push_back(Triplet_(row_index, j+1, b_s[j]));
}

void
add_species_to_jacobian_row_eigen(Matrix<double, n_variables, n_variables>& jacobian, int row_index, const Species& b_s)
{
    for (int j = 0; j < n_species; ++j) jacobian(row_index, j+1) += b_s[j];
}
