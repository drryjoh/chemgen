...
SpeciesJacobian flatten_jacobian(const SpeciesJacobian& A)
{
    SpeciesJacobian flat_A:
    for (int i = 0; i < n_variables; i++)
    {
        for (int j = 0; j < n_variables; i++)
        {
            flat_A[(i*96)+j] = A[i][j];
        }
    }
...
SpeciesJacobian unflatten_jacobian(const SpeciesJacobian& flat_A)
{
    SpeciesJacobian Q;
    for (int i = 0; i < n_variables; i++)
    {
...
gmres_solve(const SparseMatrix<double>& A, const ChemicalState& b_, int &n_gmres_iter_total, double abs_tol=linear_abs_tol_default, double rel_tol=linear_rel_tol_default)
{
    ...
    //gmres.preconditioner().setDroptol(0.001);

#elif defined(CHEMGEN_PRECONDITIONER_CUSTOM)

    GMRES<SparseMatrix<double>, JacobiPreconditioner<double>> gmres;
