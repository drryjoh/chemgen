...
RK4_STEPS = 3000
RK4_DT = 1e-8         # explicit RK4 needs a small step to stay stable at this state's fast timescales

SDIRK2_STEPS = 30
SDIRK2_DT = 1e-6      # implicit SDIRK2 can take ~100x larger steps to reach the same final time
...
def create_test(gas, chemical_mechanism, headers, test_file_name, configuration, destination_folder, n_points=0):
    ...
    with open(test_file, 'w') as file:
        ...
        concentration_test = '{species} species  = {{{array}}};'.format(
            array=','.join(["{scalar_cast}({c})".format(c=c, **vars(configuration)) for c in concentrations]),
            **vars(configuration))
...
{{
    ...
    {chemical_state} y0 = set_chemical_state(int_energy, species);

    // RK4: explicit, needs a small step to stay stable at this state's fast timescales
    {{
        {chemical_state} y = y0;
        {scalar} dt = {rk4_dt};
        {scalar} t = 0;
        std::cout << "RK4 " <<t<<" "<<temperature(y[0], y) <<" "<< get_species(y) << std::endl;
        for({index} i = 0; i < {rk4_steps}; i++)
        {{
            y = rk4(y, dt);
            t = t + dt;
            std::cout << "RK4 " <<t<<" "<<temperature(y[0], y) <<" "<< get_species(y) << std::endl;
    ...
    // SDIRK2: implicit, can take a much larger step to reach the same final time
    {{
        {chemical_state} y = y0;
        {scalar} dt = {sdirk2_dt};
        {scalar} t = 0;
        std::cout << "SDIRK2 " <<t<<" "<<temperature(y[0], y) <<" "<< get_species(y) << std::endl;
        for({index} i = 0; i < {sdirk2_steps}; i++)
        {{
            y = sdirk2(y, dt);
            t = t + dt;
            std::cout << "SDIRK2 " <<t<<" "<<temperature(y[0], y) <<" "<< get_species(y) << std::endl;
    ...
    return 0;
        ...
        rk4_dt=RK4_DT, rk4_steps=RK4_STEPS,
        sdirk2_dt=SDIRK2_DT, sdirk2_steps=SDIRK2_STEPS))
