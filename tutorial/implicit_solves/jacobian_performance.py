...
{jacobian_function} 
finite_difference_jacobian({species_parameter} species,
                           {scalar_parameter} temperature, 
                           {scalar_parameter} dx)
{{
    {jacobian} source_jacobian_ = {{}};
    {species} x_perturbed = species;

    for ({index} sp = 0; sp<n_species; sp++)
    {{
        {scalar} orig = species[sp];
        
        x_perturbed[sp] = orig + dx;
        Species forward = source(x_perturbed, temperature);

        x_perturbed[sp] = orig - dx;
        Species backward = source(x_perturbed, temperature);

        source_jacobian_[sp] = scale_gen(inv_gen(2.0 * dx), forward - backward);

        x_perturbed[sp] = orig;  // Restore
    }}
    
    return source_jacobian_;
}}
...
{{
    ...
    {{
        throw_away = source_jacobian(species, temperature_);
    }}
    auto end = std::chrono::high_resolution_clock::now();
    std::chrono::duration<double> duration1 = end - start;
    std::cout << "Source Jacobian Analytical: " << duration1.count() << " seconds" << std::endl;

    start = std::chrono::high_resolution_clock::now();
    for({index} i = 0; i < 1000; i++)
    {{
        throw_away = finite_difference_jacobian(species, temperature_, 1e-8);
    }}
    end = std::chrono::high_resolution_clock::now();
    std::chrono::duration<double> duration2 = end - start;
    std::cout << "Source Jacobian Finite Difference: " << duration2.count() << " seconds" << std::endl;
