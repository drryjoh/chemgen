...
def pressure_dependent_arrhenius_text(reaction_index, As, Bs, Es, pressures, species_names, configuration):
    ...
    choose_text +=f"\n{indentation}else {{ return arrhenius({scalar_cast}({As[-1]}), {scalar_cast}({Bs[-1]}), {scalar_cast}({Es[-1]}), temperature); }}"

    return_text ="""
{device_option}
{scalar_function}
call_forward_reaction_{reaction_index}({species_parameter} species, {scalar_parameter} temperature) {const_option}
{{
        {scalar} pressure_ = pressure(species, temperature);
		{scalar} log_pressure = log_gen(pressure_);
