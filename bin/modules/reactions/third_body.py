...
def dthird_body_text_dmixture_concentration(reaction, i, A, B, E, efficiencies, species_names, requires_mixture_concentration, dthird_body_multiplier_dspecies_index,configuration):
    mixture_concentration = "mixture_concentration"
    efficiency = 0.
    return_text = ''
    if requires_mixture_concentration:
        ...
        zero_species[dthird_body_multiplier_dspecies_index] = '{efficiency}*dthird_body_dmixture_concentration({scalar_cast}({A}), {scalar_cast}({B}), {scalar_cast}({E}), temperature, log_temperature, {mixture_concentration})'
    ...
    return return_text.format(**vars(configuration), i = i, A = A, E = E, B = B, mixture_concentration = mixture_concentration, efficiency=efficiency)
...
def create_reaction_functions_and_calls_third_body(reaction_rates, reaction_rates_derivatives, reaction_calls, reaction, configuration, reaction_index, is_reversible, requires_mixture_concentration, species_names, verbose = False, temperature_jacobian = False):
    ...
    if temperature_jacobian:
        reaction_rates_derivatives.append(dthird_body_text_dtemperature(reaction_index, reaction.rate.pre_exponential_factor, reaction.rate.temperature_exponent, reaction.rate.activation_energy, get_efficiencies(reaction), species_names, requires_mixture_concentration[reaction_index], configuration))
        reaction_rates_derivatives.append(dthird_body_text_dlog_temperature(reaction_index, reaction.rate.pre_exponential_factor, reaction.rate.temperature_exponent, reaction.rate.activation_energy, get_efficiencies(reaction), species_names, requires_mixture_concentration[reaction_index], configuration))
    else:
        reaction_rates_derivatives.append(f'//dcall_forward_reaction_{reaction_index} temperature unused not needed')
    reaction_rates_derivatives.append(dthird_body_text_dmixture_concentration(reaction, reaction_index, reaction.rate.pre_exponential_factor, reaction.rate.temperature_exponent, reaction.rate.activation_energy, get_efficiencies(reaction), species_names, requires_mixture_concentration[reaction_index], dthird_body_multiplier_dspecies_index, configuration))
