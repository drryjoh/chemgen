...
def load_custom_sourcewriter(filepath):
    """Use a custom source writer"""
    print("Experimental, see: ")
    spec = importlib.util.spec_from_file_location("source_custom", filepath)
    custom_module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(custom_module)
    return custom_module.SourceWriter

def process_cantera_file(gas, configuration, destination_folder, args, chemistry_solver, verbose=False, fit_gibbs_reaction=True, temperature_jacobian=False, remove_reactions=False, sparsity_pattern=None):
    """Processes cantera file and creates all associated variables for code generation"""
    ...
    with open(destination_folder/'source.h','w') as file:
        if args.custom_source:
            ...
            source().write_source(file, equilibrium_constants, reaction_calls, progress_rates, is_reversible, species_production_on_fly_function_texts, species_production_texts, headers, configuration, args.temperature_equation, fit_gibbs_reaction =  fit_gibbs_reaction)
