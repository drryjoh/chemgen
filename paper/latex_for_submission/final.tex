...
C++-based computational physics codes. ChemGen aims to make chemical
kinetics more accessible in existing simulation frameworks and help
bridge the gap between combustion modeling and computational physics.
The package employs the concept of decorators which enable flexible
C++ code generation to target established software ecosystems. ChemGen
...
for codes written in C, C++, and Fortran, as well as in interpreted
...
evolution. Pyrometheus generates either C++ header files or Python
...
computing in CFD. Additionally, KinetiX~\citep{Dan25} is a software
...
kinetics into C++ based codes. ChemGen introduces decorators, discussed
in detail in Section~(\ref{sec:Code-Generation}), to enable flexible
C++ code generation tailored to existing software ecosystems. ChemGen
...
fluid state. The pressure from the ideal gas law is
...
This work assumes a thermally perfect gas, with $u_{i}$ given by~\citep{Gio99}
\begin{equation}
u_{i}=h_{i}-R_{i}T=h_{\mathrm{ref},i}+\int_{T_{\mathrm{ref}}}^{T}c_{p,i}(\tau)d\tau-R_{i}T,\label{eq:internal_energy_identity}
\end{equation}
where $h_{i}$ is the mass-specific enthalpy of the $i$th specie,
$R_{i}=R/W_{i}$, $T_{\mathrm{ref}}$ is the reference temperature
(298 K), $h_{\mathrm{ref},i}$ is the reference-state enthalpy of
formation of specie $i$ evaluated as $h_{ref,i}=h_{i}\left(T_{ref}\right)$,
and $c_{p,i}$ is the mass-specific heat capacity at constant pressure
of the $i$th specie. $c_{p,i}$ is computed from an $n_{p}$-order
polynomial as
\begin{equation}
...
parametrization~\citep{Mcb93,Mcb02}. For differentiability and computational
efficiency, ChemGen fits the polynomial for $c_{p,i}$ to arbitrary
order. Equation~(\ref{eq:internal_energy_identity}) yields
...
\begin{equation}
u_{i}=h_{\text{ref},i}+\sum_{k=0}^{n_{p}}\frac{a_{ik}}{k+1}T^{k+1}-RT=\sum_{k=0}^{n_{p}+1}c_{ik}T^{k}.\label{eq:internal-energy-polynomial}
\end{equation}
Here, we use $h_{i}(T=298\text{ K})$ from the existing NASA coefficients,

\begin{equation}
h_{\text{ref},i}=h_{i}(T=298\text{ K})-\sum_{k=0}^{n_{p}}\frac{a_{ik}}{k+1}\left(298\text{ K}\right)^{k+1},\label{eq:enthalpy-adjustment}
\end{equation}
which gives
...
\end{equation}
This integration requires two vectors for temperature: a monomial
basis for polynomials of degree $n_{p}$, $T_{n_{p}}=\left(1,T,T^{2},\dots,T^{n_{p}}\right)$
and a monomial basis for polynomials of degree $n_{p}+1$, $T_{n_{p}+1}=\left(1,T,T^{2},\dots,T^{n_{p}+1}\right)$.
These vectors can then be contracted with a vector of coefficients;
for example, $u_{i}=\left<A_{i},T_{n_{p}+1}\right>$ where $A_{i}=(c_{i,0},c_{i,1},c_{i,2},\dots,c_{i,n_{p}+1})$.
...
$s_{i}^{o}$ denotes the species entropy at the reference pressure,
and $p_{i}=C_{i}R^ {}T$ is the partial pressure. The $R\log\frac{p_{i}}{p_{\mathrm{ref}}}$
term accounts for the effect of pressure on entropy and does not depend
on thermodynamic polynomial fits. Using Equation~(\ref{eq:specific_heat_polynomial}),
$s_{i}^{o}$ is computed as
...
that is calculated indepedent of these thermodynamic fits.

This gives two new temperature vectors: $T_{n_{p},\ln T}=\left(1,T,T^{2},\dots,T^{n_{p}},\ln T\right)$
and $T_{n_{p}+1,T\ln T}=\left(1,T,T^{2},\dots,T^{n_{p}+1},-T\ln T\right)$.
These vectors can then be contracted with a vector of coefficients,
$s_{i}=\left<A_{i},T_{n_{p},\ln T}\right>$ where $A_{i}=(d_{i,0},d_{i,1},d_{i,2},\dots,d_{i,n_{p}},a_{i,0})$
and $g_{i}=\left<A_{i},T_{n_{p}+1,T\ln T}\right>$ where $A_{i}=(f_{i,0},f_{i,1},f_{i,2},\dots,f_{i,n_{p}+1},a_{i,0})$.

The thermodynamic relationships used by ChemGen are summarized in
Table~(\ref{tab:Summary-of-polynomial}).
...
$h_{\text{ref},i}=h_{i}(T_{\mathrm{ref}})-\sum_{k=0}^{n_{p}}\frac{a_{ik}}{k+1}\left(298\right)^{k+1}$\tabularnewline
\hline 
...
\hline 
Species specific standard entropy, $s_{i}^{o}$ & $s_{i}^{o}=a_{i,0}\ln T+\sum_{k=0}^{n_{p}}d_{ik}T^{k}$ & $d_{ik}=\begin{cases}
\frac{a_{i,k}}{k}, & k\geq1\\
s_{\mathrm{ref},i}^{o} & k=0.
\end{cases}$\tabularnewline
\hline 
Species specific standard Gibbs
...
ChemGen supplies a Newton's method implementation to extract temperature
from internal energy in the case where temperature is needed from
$y_{f}$. For example, internal energy is typically part of a conserved
state. The total energy in $y_{f}$ is given as

\begin{equation}
\rho e_{t}=\rho u+\frac{1}{2}\sum_{k=1}^{d}\rho v_{k}v_{k},
\end{equation}
where the term $\frac{1}{2}\sum_{k=1}^{d}\rho v_{k}v_{k}$ can be
computed from other conserved variables. Thus one can directly compute
the internal energy at any time as

\begin{equation}
\rho u=\frac{1}{2}\sum_{k=1}^{d}\frac{\left(\rho v_{k}\right)\left(\rho v_{k}\right)}{\sum_{i}^{n_{s}}W_{i}C_{i}}-\rho e_{t}.
\end{equation}

The temperature is determined by a root finding algorithm which iterates
via Netwon's method. The temperature at the $\left(k+1\right)$th
Newton iteration is given by

\begin{equation}
T_{k+1}=T_{k}-\frac{\rho u(T_{k})}{\frac{\partial\rho u(T_{k})}{\partial T}}.\label{eq:netwon_iteration_temperature}
\end{equation}
where $T_{k}$ is the temperature at the $k$th iteration. The numerator
and denominator are given by

\begin{equation}
\rho u(C,T_{k})=\sum_{i=1}^{n_{s}}W_{i}C_{i}u_{i}(T_{k})=\sum_{i=1}^{n_{s}}W_{i}C_{i}\left<A_{i},T_{k,n_{p}+1}\right>
\end{equation}
 and

\begin{equation}
\frac{\partial\rho u(C,T_{k})}{\partial T}=\rho c_{v}\left(C,T_{k}\right)=\sum_{i=1}^{n_{s}}W_{i}C_{i}\frac{\partial u_{i}(T_{k})}{\partial T}=\sum_{i=1}^{n_{s}}W_{i}C_{i}\left<A_{i},\frac{\partial T_{k,n_{p}+1}}{\partial T}\right>,\label{eq:denominator_for_temp_solve}
\end{equation}
respectively, where
...
\begin{equation}
\frac{\partial T_{k,n_{p}+1}}{\partial T}=\left(0,1,2T_{k},\dots,\left(n_{p}+1\right)T_{k}^{n_{p}}\right).
\end{equation}
$c_{v}$ in Equation~\ref{eq:denominator_for_temp_solve} is the
specific heat at constant volume, which is related to $c_{p}$ as

\begin{equation}
c_{v}=c_{p}-R.\label{eq:cv_equation}
\end{equation}
In ChemGen, Newton's method for computing temperature terminates after
five iterations, which in our experience is sufficient to be within
$1\times10^{-8}$ relative error for a variety of models at different
conditions.
...
chemical state and retain the stability of the fluid dynamic algorithms~\citep{Str68,Hou11,Lv15,Joh20}.
...
C++ is a malleable language that allows developers to abstract underlying
...
can be embedded into all existing C++ software.
...
generating desirable C++ code. The above example can be changed to
a Python formattable Python string as
...
readable than symbolic differentiation using tools such as SymPy~\citep{sympy}.
...
process. Regardless, since the functions are available in C++, packages
...
file, generates C++ code in the current directory in a \texttt{src}
...
Finally, Niemeyer et al.~\citep{Nie17} reported error metrics that
...
\begin{figure}[H]
\begin{centering}
\includegraphics[scale=0.4]{renamed_images/image_5.png}
\par\end{centering}
...
code generation to integrate multispecies thermodynamics and chemical
kinetics into C++-based computational physics software, with an emphasis
on CFD applications. ChemGen employs a decorator-based approach to
enable flexible and modular C++ code generation tailored to existing
