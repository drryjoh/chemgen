...
def create_test(gas, chemical_mechanism, headers, test_file_name, configuration, destination_folder, n_points = 1):
    ...
    mech_name  = chemical_mechanism.split('.yaml')[0]
    mech_name = mech_name.replace('./','')
...
void max_err({scalar_parameter} temperature, {species_parameter} result, {species_parameter} cantera_source, {species_parameter} concentrations,  std::ofstream& file) 
{{
    ...
    {{
        //weight = concentrations[i]/sum_c;
        l2_norm = std::max(std::sqrt(pow_gen2(safe_divide(result[i]- cantera_source[i], cantera_source[i]))), l2_norm);
...
{index} main() {{
    ...
    std::ofstream file_max("max_{mech_name}.csv");
    std::ofstream states("{mech_name}.csv");
    if (!file.is_open()) 
    {{
