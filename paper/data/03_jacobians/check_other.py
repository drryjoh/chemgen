...
def random_yc(ns):
    ...
    species_array = np.ones(ns)
    species_array /= species_array.sum()

    return (1000 + 1500 * np.random.random(), 10132.5 + 101325.0 * 9.9 * np.random.random(), species_array)
    # return (1000, 10132.5, species_array)

def discrete_jacobian(shape,C,T,dc):
    n_species  = len(C)
    J = np.zeros(shape)
    for i in range(n_species):
        C[i] = C[i] + dc
        S1 = np.array(cg.temperature(C, T))
        C[i] = C[i] - dc
        C[i] = C[i] - dc
        S2 = np.array(cg.temperature(C, T))
        C[i] = C[i] + dc
        J[:,i] = (S1-S2) / (2 * dc)
    return J

def discrete_jacobian_new(shape,C,T,dc):
    n_species  = len(C)
    J = np.zeros(shape)
    rho_u = cg.internal_energy_volume_specific(C, T)
    for i in range(n_species):
        C[i] = C[i] + dc
        S1 = np.array(cg.temperature(C, rho_u))
        C[i] = C[i] - dc
        C[i] = C[i] - dc
        S2 = np.array(cg.temperature(C, rho_u))
        C[i] = C[i] + dc
        J[:,i] = (S1-S2) / (2 * dc)
        # breakpoint()
    return J
...
def L2_J(Jcg, Jfd, ns):
    L2 = 0
    L2_J = Frobenius(Jcg)
    for i in range(Jcg.shape[0]):
        for j in range(Jcg.shape[1]):
            if np.abs(Jcg[i,j]) > L2_J/(10**20):
...
temperature_jacobian = -(gas.standard_int_energies_RT*T*ct.gas_constant)/(gas.density*gas.cv_mass)

print("gas.density*gas.int_energy_mass = %g" % (gas.density*gas.int_energy_mass))
...
C = yci[1:]
dc = np.min(C[C > 0])/10
Jcg = np.array(cg.dtemperature_dspecies(C,T))
if Jcg.ndim == 1:
    Jcg.shape = (1, Jcg.shape[0])
elif Jcg.ndim == 2:
    Jcg.shape = (1, 1)

for r in range(4):
    Jfd = discrete_jacobian_new(Jcg.shape, C, T, dc/(2**r))
