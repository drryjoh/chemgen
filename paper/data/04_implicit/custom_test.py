...
std::ostream& operator<<(std::ostream& os, const std::array<T, N>& arr) {
    ...
    return os;
        ...
        content = """
auto read_scalar_or_default = [](const YAML::Node& node, const std::string& key, {scalar} default_value) 
{{
...
auto read_index_or_default = [](const YAML::Node& node, const std::string& key, {index} default_value) 
{{
    if (node[key]) return node[key].as<{index}>();
    std::cerr << "[Warning] " << key << " not defined. Using default: " << default_value << "\\n";
    return default_value;
}};
...
{{
    ...
    for({index} i = 0; i < n_run; i++)
    {{
        y = sdirk4(y, dt, 1e-10, max_iter_sdirk4);
        t = t + dt;
        sdirk4_file << t << " " << temperature(y);
        for (const auto& val : get_species(y)) sdirk4_file << " " << val;
        sdirk4_file << "\\n";
