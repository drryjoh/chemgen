...
def backwards_euler(C, T, dt, cg, formulation, gmres_tolerance=1e-6, max_iter=20):
    """
    ...
    y[1:] = C
    if formulation == "cons":
        y[0] = float(cg.internal_energy_volume_specific(C, T))
    else:
        y[0] = float(T)
    ...
    for _ in range(max_iter):
        T_ = T
        if formulation == "cons":
            T_ = cg.temperature(y_guess[1:], y_guess[0])
        else:
            T_ = y_guess[0]
        f = np.array(cg.source(y_guess[1:], T_), dtype=float)
        J = np.array(cg.source_jacobian(y_guess[1:], T_), dtype=float)
        A = I / dt - J
        ...
        if gmres_method == "numpy":
            ...
            r = eigen_gmres.gmres_dense(A, res, rtol=1e-4, maxiter=100, restart=50)
