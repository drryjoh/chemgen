...
vals = [1/np.load(f"phi_{phi}_p{p}_linear.npy") for phi in phis for p in pressures_atm]
...
for i, phi in enumerate(phis):
    for j, p_atm in enumerate(pressures_atm):
        y = np.load(f"phi_{phi}_p{p_atm}_linear.npy")
...
cbar.set_label(
    r"$n_{\text{l,cons}}/n_{\text{l,temp}}$",
    fontsize=16, labelpad=10
)

fig.subplots_adjust(hspace=0.35, wspace=0.25, top=0.75)
plt.savefig("stiffness_parameter_space_linear_threshold.png", dpi=300)
plt.show()
