...
for t in time:
    ...
    y_cgc, n_cgc, n_cgc_newt  = cgc.sdirk2(C, T, dt)
    y_cnt, n_cgt, n_cgt_newt = cgt.sdirk2(C, T, dt)
...
axs[0].plot(time, iterations_cgt, color=colors["orange"],linestyle = '--', label = "Temperature Formulation")
axs[0].plot(time, iterations_cgc, color=colors["red"], linestyle = '-.', label = "Conservative Formulation")
axs[0].legend()
